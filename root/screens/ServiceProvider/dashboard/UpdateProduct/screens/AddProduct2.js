...
export default function AddProduct2({navigation, ...props}) {
    const mediaFiles=props.route.params.params.media.map(item=>({file:{uri:item.file},id:item.id}))
...
console.log(mediaFiles);
    ...
    const [media, setMedia] = React.useState([...mediaFiles]);
    const session = useContext(SessionContext)
    const [mediaID, setMediaID] = React.useState([...mediaFiles?.map(item=>item.id)]);
    ...
    let handleContinue = ()=>{
        if(buttonText==="Continue"){
            if(validate()){
                ...
                navigation.navigate("UpdateProduct3",{form:data,params:props.route.params.params})
            }
        }else{
    ...
    return (
        <View style={[styles.container]}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={[styles.contentContainer]}>
                ...
                <View style={[MARGINS.v22, FLEX_STYLE.row,media?.length>2&&FLEX_STYLE.center,{columnGap:10}, FLEX_STYLE.rap]}>
